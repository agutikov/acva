...
- Decision: ship `cfg.apm.use_system_aec: true` as the default. The in-process WebRTC APM wiring (M6 § 5.1) is correct but ineffective on the dev laptop's ALC257 codec + class-D speaker chain (M6 § 5.2 + `docs/reports/aec_report.md` § 6). PipeWire's `module-echo-cancel` consumes the raw ALSA mic stream upstream of the codec DSP and gives 25–46 dB speech-band cancellation on the same hardware (M6B Step 4.1).
...
- Path A (in-process APM) stays compiled and gated by `cfg.apm.aec_enabled`. Both stacks running simultaneously is documented as a misconfiguration (NS over-suppresses, two AGCs fight, AEC convergence on a pre-cleaned signal zeroes the first ~800 ms of every utterance — observed during M6B exploration; see `docs/reports/aec_report.md` § 10.5).
...
**L8. M8C wake-word ships off-by-default; address detection (M10) is the real conversational gate.** *(resolved 2026-05-08)*
- Background: M8C Tier 1–3 landed code-complete (`src/audio/wake_word.cpp`, `acva demo wake-word`, `tools/train-wake-word`, observability counters, hot-reloadable threshold). The new `acva demo wake-word-offline --wav <path>` + `scripts/wake-word-offline.sh` debug pair confirmed the engine runs correctly on real audio: openWakeWord stock `hey-jarvis` peaks 0.37–0.47 on Russian-accented "hey jarvis" — under the 0.55 default but well above the noise floor, so the model behaves as documented. STT in parallel transcribed the same phrase cleanly.
- Five-point review of whether wake-word is the right primitive for acva's product line:
  1. **VRAM/cost.** The generic "wake-word saves VRAM by gating STT loading" argument doesn't apply here. Speaches stays pinned (`WHISPER__TTL=-1` per L7) so STT is in VRAM 24/7 anyway. Wake-word saves only *inference* compute on background utterances — near-free on an idle GPU.
  2. **VAD vs wake-word.** Whisper hallucinations on silence (the `"Субтитры делал DimaTorzok"` family) are a VAD-failure symptom: if the endpointer doesn't deliver silence to STT, those hallucinations don't happen. Wake-word doesn't *fix* VAD; it sits as a separate downstream filter ("was this speech addressed to me?"), and openWakeWord's phonetic matcher doesn't actually distinguish "you said it" from "the TV said it".
  3. **UX mode.** acva's product line is "multi-hour conversations" (CLAUDE.md project-line). That's the always-listening conversational-agent mode, not the press-button-and-ask assistant mode. Requiring "hey jarvis" before every turn is wrong UX for that target. The right gate is **address detection** ("is the user addressing me right now, or someone else / themselves / the room?"), which is M10 in `project_design.md` §17.
  4. **Privacy.** Mode question, not a wake-word property. A continuously listening conversational agent has already accepted continuous local transcription; the privacy boundary is "stays on-device", not "we don't transcribe."
  5. **Latency.** Live demo measurements supersede generic claims; not a wake-word distinguisher in this codebase.
- **Decision:** ship `cfg.audio.wake_word.enabled: false` as the default. The framework, demos, and trainer stay built and tested — they're correct code, useful for sleep-from-idle modes and as the offline-diagnostic harness, and they're the dependency runway for M10 (address detection will likely reuse the gate plumbing in `pipeline.cpp:174-208`). M8C closes as code-complete; the remaining wake-word-on-by-default work is dropped from the roadmap.
- **What this means for M10:** address detection is now the load-bearing conversational-mode gate, not a stretch goal. The wake-word gate semantics in `audio/pipeline.cpp` (`gate_is_open` + `followup_window_ms` + the SpeechStarted suppression) are the seam M10 will plug into; the model behind the boolean changes from "phonetic match" to "addressed-to-me classifier" but the orchestration above stays. M10 specs in `project_design.md` §17 should be updated to reference this seam explicitly when M10 starts.
- **What ships in default config now:** wake-word block stays in `config/default.yaml` for documentation + opt-in operators; only `enabled` flips to `false`. The `models:` registry catalog and `tools/acva-models install hey-jarvis` flow are unchanged — operators who want sleep-and-wake mode get a one-line config edit, not a re-install.
